...
let block = {
  ...
  drawBlock: function () {
    ...
    if (
      block.position.x >= canvas.width - block.size ||
      block.position.x <= block.size
    )
      block.acceleration.x = -block.acceleration.x;
    if (
      block.position.y >= canvas.height - block.size ||
      block.position.y <= block.size
    )
...
function startMessage() {
  ctx.font = "20px white";
  ctx.fillText(
    "Save the block! Use arrow keys to dodge",
    canvas.width / 3.5,
    20
  );
}
...
window.addEventListener("keydown", function (event) {
  ...
  if (j == 0) {
    ...
    if (event.keyCode == 39 && blockm.position.x < canvas.width - blockm.size)
      ...
      blockm.position.x += Math.min(
        15,
        canvas.width - blockm.position.x - blockm.size
      );
    // move Left
    ...
    // move down
      ...
      event.keyCode == 40 &&
      blockm.position.y < canvas.height - blockm.size
    )
      // updates location by int given
